...
function getInfo(name){
	...
	var result = String(get_url(request));
	var myObjectJSON = JSON.parse(httpGet(result));
	var infoJSON = {image:'',description:''};
	var image = myObjectJSON.results.bindings[0].image.value;
	var description = myObjectJSON.results.bindings[0].description.value;
	if(image !== '')
		infoJSON.image = image;
	if(description !== '')
